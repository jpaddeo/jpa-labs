...
class MySqlDbClass { 
   ...
   var $last_errorno;         // holds the last error. Usually mysql_error()
   ...
   function select_db($db='') {
      ...
      $prim_log->logWrite("","class.mysql.db -> select_db :" . $this->db);
      if (!mysql_select_db($this->db)) {
         $this->last_error = mysql_error();
         $this->last_errorno = mysql_errno();
         $prim_log->logWrite("ERROR", $this->last_error . "(" .$this->last_errorno . ")");
         return false;
      }
      return true;
   }
   ...
   function select($sql) {
      ...
      $prim_log->logWrite("","class.mysql.db -> select :" . $sql);
      $r = mysql_query($sql);
      if (!$r) {
         $this->last_error = mysql_error();
         $this->last_errorno = mysql_errno();
   ...
   function select_one($sql) {
      ...
      $prim_log->logWrite("","class.mysql.db -> select_one :" . $sql);
      $r = mysql_query($sql);
      if (!$r) {
         $this->last_error = mysql_error();
         $this->last_errorno = mysql_errno();
   ...
   function insert_sql($sql) {
      ...
      $prim_log->logWrite("","class.mysql.db -> insert_sql :" . $sql);
      $r = mysql_query($sql);
      if (!$r) {
         $this->last_error = mysql_error();
         $this->last_errorno = mysql_errno();
   ...
   function update_sql($sql) {
      ...
      $prim_log->logWrite("","class.mysql.db -> update_sql :" . $sql);
      $r = mysql_query($sql);
      if (!$r) {
         $this->last_error = mysql_error();
         $this->last_errorno = mysql_errno();
   ...
   function delete_sql($sql) {
       global $prim_log;
      // Deletes data in the database via SQL query.
      // Returns the number or row affected or true if no rows needed the update.
      // Returns false if there is an error.
      $this->last_query = $sql;
      
      $prim_log->logWrite("","class.mysql.db -> delete_sql :" . $sql);
      $r = mysql_query($sql);
      if (!$r) {
         $this->last_error = mysql_error();
         $this->last_errorno = mysql_errno();
   ...
   function print_last_error($show_query=true) {
      ...
      echo "<div style='border: 1px solid red; font-size: 9pt; font-family: monospace; color: red; padding: .5em; margin: 8px; background-color: #FFE2E2'>";
        echo "<span style='font-weight: bold'>db.class.php Error:</span><br>" . $this->last_error;
      echo "</div>";
   ...
   function get_last_errorno() {
      return $this->last_errorno;
   }
   ...
   function print_last_query() {
      ...
      // box.
      echo "<div style='border: 1px solid blue; font-size: 9pt; font-family: monospace; color: blue; padding: .5em; margin: 8px; background-color: #E6E5FF'>";
